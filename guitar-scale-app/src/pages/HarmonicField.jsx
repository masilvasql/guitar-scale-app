...
function createEmptyTable() {
  return Array.from({ length: ROWS_COUNT }, () =>
    Array.from({ length: INTERVALS.length }, () => '')
  )
}

function HarmonicField() {
  ...
  return (
    <div className="hf-page">
      <header className="hf-header">
        ...
        <p className="hf-subtitle">Preencha os acordes de cada grau do campo harmônico</p>
      </header>

      <div className="hf-controls">
        <button
          className="hf-btn hf-btn-clear"
        ...
        <button
          className="hf-btn hf-btn-download"
      ...
      <div className="hf-name-container">
        <input
          type="text"
          className="hf-name-input"
          placeholder="Digite um título... Ex: Campo Harmônico de C Maior"
          value={tableName}
          onChange={(e) => setTableName(e.target.value)}
        />
      </div>

      <div ref={captureRef} className="hf-capture-area">
        {tableName.trim() && (
          <div className="hf-table-title">
            <h2>{tableName}</h2>
          </div>
        )}
        ...
        <div className="hf-table-wrapper">
          <table className="hf-table">
            <thead>
              <tr>
                <th className="hf-row-number">#</th>
                {INTERVALS.map((interval, i) => (
                  <th key={i} className="hf-interval-header">{interval}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {tableData.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  <td className="hf-row-number">{rowIndex + 1}</td>
                  {row.map((cell, colIndex) => (
                    <td key={colIndex} className="hf-cell">
                      <input
                        type="text"
                        className="hf-cell-input"
                        value={cell}
                        onChange={(e) =>
